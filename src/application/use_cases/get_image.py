...
class GetImageUseCase:
    ...
    async def execute(self, image_id: uuid.UUID) -> ImageResponse | None:
        ...
        if image is None:
            logger.debug("Image not found in repository: %s", image_id)
            return None
    ...
    async def get_file(self, image_id: uuid.UUID, thumbnail: bool = False) -> bytes | None:
        image = await self._repository.get_by_id(image_id)
        if image is None:
            logger.debug("Image not found for file download: %s", image_id)
            return None
        path = image.thumbnail_path if thumbnail else image.original_path
        if path is None:
            kind = "thumbnail" if thumbnail else "original"
            logger.debug("No %s path for image %s", kind, image_id)
            return None
